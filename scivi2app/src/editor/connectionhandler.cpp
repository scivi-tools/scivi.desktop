...
ConnectionHandler::ConnectionHandler(GraphView *graphView)
    : m_graphView(graphView) {
    QObject::connect(graphView, &GraphView::inputClicked, this,
                     &ConnectionHandler::onInputClicked);
    QObject::connect(graphView, &GraphView::outputClicked, this,
                     &ConnectionHandler::onOutputClicked);
    QObject::connect(graphView, &GraphView::inputEntered, this,
                     &ConnectionHandler::onInputEntered);
    QObject::connect(graphView, &GraphView::spaceClicked, this,
                     &ConnectionHandler::resetConnectionState);
}

bool ConnectionHandler::isActive() const { return !m_tempEdge.isNull(); }

void ConnectionHandler::onInputClicked(NodeView *node, QQuickItem *socketView,
                                       NodeSocket *socket) {
    Q_UNUSED(node);
    Q_UNUSED(socketView);
...
void ConnectionHandler::onOutputClicked(NodeView *node, QQuickItem *socketView,
                                        NodeSocket *socket) {
    Q_UNUSED(node);
    Q_UNUSED(socketView);
    Q_ASSERT(socket != nullptr);
...
void ConnectionHandler::connectDestWithMousePosition(EdgeView *edgeView) {
    connect(
        m_graphView, &GraphView::mousePositionChanged, this,
        [=](float x, float y) { edgeView->setDestPointGlobal(QPointF(x, y)); });
}
