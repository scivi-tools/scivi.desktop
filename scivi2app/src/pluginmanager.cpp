...
PluginManager::PluginManager(Editor *editor, QQmlEngine *engine, QObject *parent)
...
Plugin* PluginManager::load(QString pathToPlugin)
{
    ...
    if (!instance) {
        qWarning() << "Couldn't load plugin: " << pathToPlugin << ". Error: " << loader.errorString();
    }
...
QList<Plugin *> PluginManager::findPluginsInDir(QDir dir)
{
    ...
    for (const auto &pluginDirName: dir.entryList(QDir::Dirs | QDir::NoDotAndDotDot)) {
