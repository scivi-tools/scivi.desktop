...
scivi::widget::ZoomPanView::ZoomPanView(QQuickItem *parent)
    : QQuickItem(parent) {
...
bool scivi::widget::ZoomPanView::panEnabled() const { return m_panEnabled; }
...
void scivi::widget::ZoomPanView::wheelEvent(QWheelEvent *event) {
    ...
    qreal newScale =
        oldScale + (event->angleDelta().y() > 0 ? zoomDiff : -zoomDiff);
    if (newScale < zoomMin) return;
    QPointF wheelTargetPoint(static_cast<qreal>(event->x()),
