...
SciViApplication::SciViApplication(QQmlApplicationEngine &engine)
    : m_editor(QSharedPointer<Editor>::create(&engine)),
      m_pluginManager(QSharedPointer<PluginManager>::create(m_editor.data(), &engine))
{   

}

void SciViApplication::initialize(Settings &settings, QApplication &app, QQmlApplicationEngine &engine)
{
    ...
    QCommandLineOption ontologyOpt("ont", tr("Read ontology from <file>."), "file");
    ...
    engine.rootContext()->setContextProperty("logger", &StatusBarLogger::instance());
    ...
    engine.rootContext()->setContextProperty("pluginManager", m_pluginManager.data());

    qRegisterMetaType<scivi::diagram::Node*>("Node*");
    qRegisterMetaType<scivi::diagram::Edge*>("Edge*");
    qmlRegisterUncreatableType<scivi::diagram::NodeSocket>("scivi", 0, 1, "NodeSocket", "cannot create nodesocket");
...
int SciViApplication::run(QApplication &app)
{
    ...
    if (result == QtAndroid::PermissionResult::Denied) {
        ...
        if (resultHash[writeExternalStorage] == QtAndroid::PermissionResult::Denied) {
            return 0;
        }
    }
#endif
