...
void GraphView::setDiagram(DataflowDiagram *diagram)
{
    ...
    connect(diagram, &diagram::DataflowDiagram::nodeInserted, this, &GraphView::onNodeInserted);
}

DataflowDiagram *GraphView::diagram()
{
    return m_diagram;
}
...
std::unique_ptr<QQmlComponent> GraphView::createComponent(const QString &url)
{
    ...
    if (engine != nullptr) {
        try {
            ...
            if (!component->isReady() || component->isError() || component->isNull()) {
...
QQuickItem *GraphView::createFromComponent(QQmlComponent *component, Node *node, Edge *edge)
{
    ...
    try {
        ...
        if (object == nullptr || component->isError()) {
            if (object != nullptr)
                object->deleteLater();
            throw Error { "Failed create concrete QQuickItem from QML Component " + component->errorString() };
        }
        if (node != nullptr) {
